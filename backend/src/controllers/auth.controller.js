      ...
      try {
        ...
        if (user) {
          ...
          if (body.password !== body.password2) {
            ...
            const content = `<a href="https://simple-admin-webpage.vercel.app/auth/account-activate?email=${body.email}">Click me to activate your account!</a><br/><p>Activation Code: ${activationCode}</p>`;
  ...
  async handleResetPassword(req, res, next) {
    ...
    if (body) {
      try {
        ...
        const content = `<a href="https://simple-admin-webpage.vercel.app/auth/login">You have reset your password. Click me to log in and have fun!</a>`;
        await sendMail(
          req.query.email,
    ...
    try {
      ...
      if (body) {
        ...
        if (user) {
          ...
          const content = `<a href="https://simple-admin-webpage.vercel.app/auth/reset-password?email=${body.email}&reset_token=${reset_token}">Click me to reset your password!</a>`;
          const info = await sendMail(
            body.email,
